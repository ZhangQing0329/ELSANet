...
def train(Dataset, Network):
    ## dataset
    cfg    = Dataset.Config(datapath='/home/linyi/Desktop/datasets/train_nosize_noexp/',listpath = '/home/linyi/Desktop/datasets/train_pair_edge.lst',
                            savepath='../ablation_pth/33', mode='train', batch=6, lr=0.005, momen=0.9, decay=0.0005, epoch=70,iter_size=10,show_every =400)
    ...
    for epoch in range(cfg.epoch):
        ...
        for step, (image, mask, edge) in enumerate(loader):
            ...
            optimizer.zero_grad()
            with autocast():
                sal1_pred, sal2_pred, sal3_pred, sal4_pred, sal5_pred, edge1_pred, edge2_pred, edge3_pred = net(
                    image)
                ...
                sal_loss_fuse = sal5_loss_fuse + sal4_loss_fuse + sal3_loss_fuse + sal2_loss_fuse + sal1_loss_fuse
                ...
                edge_loss_fuse = edge1_loss + edge2_loss + edge3_loss
        ...
        loss_log = {'epoch': epoch, 'edge_loss': edge_loss_fuse.item(), 'sal_loss': sal_loss_fuse.item(), 'total_loss': loss.item()}
        log_str = json.dumps(loss_log)
        print(log_str)
        with open('./log_loss.txt', 'a') as fh:
                	fh.write(log_str+'\n')
        if epoch == 0 or epoch >=55:
